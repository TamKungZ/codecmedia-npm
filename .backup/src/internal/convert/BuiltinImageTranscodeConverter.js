...
function decodeBaselineJpegToRgb(bytes) {
  ...
  try {
    ...
    while (i < data.length) {
      ...
      const marker = data[markerIndex] & 0xff;
      i = markerIndex + 1;

      // Fill byte inside entropy-coded data (should not happen in marker scanning path).
      if (marker === 0x00) {
        continue;
      }
      ...
      if (i + 2 > data.length) {
        throw new CodecMediaException("Invalid JPEG marker length");
      }
      const segmentLength = readU16BE(data, i);
      if (segmentLength < 2) {
      ...
      switch (marker) {
        ...
        case 0xc2: // SOF2 (progressive)
          throw new CodecMediaException("Unsupported JPEG scan mode (progressive JPEG is not supported)");
        case 0xdd: // DRI
          if (segment.length !== 2) {
        ...
        case 0xda: { // SOS
          if (!frame) {
            throw new CodecMediaException("Invalid JPEG: SOS appears before SOF0");
          }
          const scan = parseSos(segment, frame, huffmanDc, huffmanAc);
        ...
        default:
          // APPn / COM / unsupported markers: ignored.
          break;
      }
    ...
    throw new CodecMediaException("Unsupported or invalid JPEG data");
  } catch (err) {
    if (err instanceof CodecMediaException) throw err;
